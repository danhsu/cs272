		...
		if (theOne == null) {
			...
			try {
				...
				for (ArrayList<String> eachItem : resultsList) 
				{
					...
					String email = eachItem.get(2);
					String role = eachItem.get(3);
				...
				for (ArrayList<String> eachItem : resultsList) 
				{
					int id = Integer.parseInt(eachItem.get(0));
					String name = eachItem.get(1);
					String platform = eachItem.get(2);
					int year = Integer.parseInt(eachItem.get(3));
					String genre = eachItem.get(4);
					String publisher = eachItem.get(5);
		...
		for (User u : theOne.mAllUsersList)
			if (u.getEmail().equalsIgnoreCase(email))
			{
				try {
					...
					// resultsList,get(0) is the row of user at 0
					ArrayList<ArrayList<String>> resultsList = theOne.mUserDB.getRecord(String.valueOf(u.getId()));
					String storedPassword = resultsList.get(0).get(4); //this is where the Password is stored.
	...
	public ObservableList<VideoGame> getGamesForCurrentUser()
	{
		...
		if (mCurrentUser != null)
		{
			try 
			{
				...
				// so use another for each loop to go through the inner ArrayList<String>
				ArrayList<ArrayList<String>> resultsList = theOne.mUserGamesDB.getRecord(String.valueOf(mCurrentUser.getId()));

				for (ArrayList<String> eachRow: resultsList)
				{
					int gameId = Integer.parseInt(eachRow.get(1));
					for (VideoGame vg : theOne.mAllGamesList)
						if (vg.getId() == gameId)
